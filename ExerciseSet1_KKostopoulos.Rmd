...
1. The code, specifically dataset1, has specified two stocks to be shown SPY and AAPL. 

2. The cumulative return for Apple is `r round(100*sum(StockReturns[,2]),1)` 

The average return for Apple is `r round(100*mean(StockReturns[,2]),3)`

The standard deviation for Apple is `r round(100*sd(StockReturns[,2]),3)`

3. The cumulative return for Yahoo is `r round(100*sum(StockReturns[,3]),1)` 

The average return for Yahoo is `r round(100*mean(StockReturns[,3]),3)`

The standard deviation for Yahoo is `r round(100*sd(StockReturns[,3]),3)`
...
1. In order to get the number of columns we can use the command ncol(transposedData) and the command nrow(transposedData) to get the nubmer of rows. For transposedData we have to get `r ncol(t(StockReturns))` columns and `r nrow(t(StockReturns))` rows. 
...
1. To get data starting with 2001-01-01 we change the StartDate="2001-01-01". In order to get the new exercise set you will need to source dataSet1.R again through the command source("dataSet1.R")
...
1. The command returns `r sum(myData != StockReturns[1:20,])`. This is because there are 20 exact matches between myData and StockReturns.

2. It will still return 20.
